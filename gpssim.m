...
sysConfig.MAX_SAT  = 32;
sysConfig.MAX_CHAN = 11;
...
llh = llh / sysConfig.R2D; 
% xyz = llh2xyz(llh);

% outfile = 
% duration = atof(optarg);

iduration    = fix(duration*10.0 + 0.5);
samp_freq    = floor(samp_freq/10.0);
iq_buff_size = samp_freq;
samp_freq    = samp_freq*10.0;
...
[~, chan, allocatedSat]= allocateChannel(chan, eph(ieph:end,:), ionoutc, grx, xyz(1,:), allocatedSat, sysConfig);
...
grx = incGpsTime(grx, 0.1, sysConfig);
...
for iumd = 1:numd - 1
    ...
    %           rho = struct('azel', zeros(1, 2)); % Initialize rho with azel field  
                ...
                chan(i) = computeCodePhase(chan(i), rho, 0.1, sysConfig);  
                chan(i).carr_phasestep = int32(round(512.0 * 65536.0 * chan(i).f_carr * delt));
    ...
    for isamp = 0:iq_buff_size - 1
            i_acc = 0;
            q_acc = 0;   
            for i = 1:sysConfig.MAX_CHAN  
                if chan(i).prn > 0  
                    % Compute iTable  
                    iTable = bitand(uint32((chan(i).carr_phase /2^16)) , 511); % 511 == 0x1ff
                    % Compute ip and qp  
                    ...
                    % Update code phase  
                    chan(i).code_phase = chan(i).code_phase + chan(i).f_code * delt;  

                    if chan(i).code_phase >= sysConfig.CA_SEQ_LEN  
                        chan(i).code_phase = chan(i).code_phase - sysConfig.CA_SEQ_LEN;  
                        chan(i).icode = chan(i).icode + 1;  

                        if chan(i).icode >= 20 % 20 C/A codes = 1 navigation data bit  
                            chan(i).icode = 0;  
                            chan(i).ibit = chan(i).ibit + 1;  

                            if chan(i).ibit >= 30 % 30 navigation data bits = 1 word  
                                chan(i).ibit = 0;  
                                chan(i).iword = chan(i).iword + 1;  
                                % if (chan(i).iword>=N_DWRD) fprintf(stderr, "\nWARNING: Subframe word buffer overflow.\n");  
                                % Set new navigation data bit  
                                chan(i).dataBit = bitget(chan(i).dwrd(chan(i).iword), 29 - chan(i).ibit) * 2 - 1; % (int)((chan[i].dwrd[chan[i].iword]>>(29-chan[i].ibit)) & 0x1UL)*2-1;  
                            end  
                        end  
                    end  

                    % Set current code chip  
                    chan(i).codeCA = chan(i).ca(fix(chan(i).code_phase + 1)) * 2 - 1; 
                ...
                % Scaled by 2^7  
                i_acc = (i_acc + 64) / 2^7; % (i_acc+64)>>7;  
                q_acc = (q_acc + 64) / 2^7; % (q_acc+64)>>7;
                iq_buff(isamp*2 + 1) = int16(i_acc);
                iq_buff(isamp*2 + 2) = int16(q_acc);
    end
    igrx = int32(grx.sec*10.0+0.5);
    if mod(igrx,300) == 0 % Every 30 seconds  
        ...
        if ~staticLocationMode  
            [~, chan, allocatedSat] = allocateChannel(chan, eph(ieph), ionoutc, grx, xyz(iumd + 1), allocatedSat, sysConfig);  
        else  
    ...
    grx = incGpsTime(grx, 0.1, sysConfig);  
    % Update time counter  
    fprintf('\rTime into run = %4.1f', subGpsTime(grx, g0, sysConfig));
