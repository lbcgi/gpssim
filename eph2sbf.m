...
function sbf = eph2sbf(eph, ionoutc, sysConfig)
    ...
    sbf4_page18_svId = 56;  
    wn = 0; % 初始化为0  
    toe = uint32(fix(eph.toe.sec/16.0));    
    toc = uint32(fix(eph.toc.sec/16.0));    
    iode = uint32(fix(eph.iode));    
    iodc = uint32(fix(eph.iodc));    
    deltan = int32(fix(eph.deltan/sysConfig.POW2_M43/pi));    
    cuc = int32(fix(eph.cuc/sysConfig.POW2_M29));    
    cus = int32(fix(eph.cus/sysConfig.POW2_M29));    
    cic = int32(fix(eph.cic/sysConfig.POW2_M29));    
    cis = int32(fix(eph.cis/sysConfig.POW2_M29));    
    crc = int32(fix(eph.crc/sysConfig.POW2_M5));    
    crs = int32(fix(eph.crs/sysConfig.POW2_M5));    
    ecc = uint32(fix(eph.ecc/sysConfig.POW2_M33));    
    sqrta = uint32(fix(eph.sqrta/sysConfig.POW2_M19));    
    m0 = int32(fix(eph.m0/sysConfig.POW2_M31/pi));    
    omg0 = int32(fix(eph.omg0/sysConfig.POW2_M31/pi));    
    inc0 = int32(fix(eph.inc0/sysConfig.POW2_M31/pi));    
    aop = int32(fix(eph.aop/sysConfig.POW2_M31/pi));    
    omgdot = int32(fix(eph.omgdot/sysConfig.POW2_M43/pi));    
    idot = int32(fix(eph.idot/sysConfig.POW2_M43/pi));    
    af0 = int32(fix(eph.af0/sysConfig.POW2_M31));    
    af1 = int32(fix(eph.af1/sysConfig.POW2_M43));    
    af2 = int32(fix(eph.af2/sysConfig.POW2_M55));    
    tgd = int32(fix(eph.tgd/sysConfig.POW2_M31)); 
    svhlth = uint32(fix(eph.svhlth));  
    codeL2 = uint32(fix(eph.codeL2));  

    wna = uint32(fix(mod(eph.toe.week,256)));  
    toa = uint32(fix(eph.toe.sec/4096.0));  
    ...
    dtls = int32(fix(ionoutc.dtls));  
    tot = uint32(fix(ionoutc.tot/4096));  
    wnt = uint32(fix(mod(ionoutc.wnt,256))); 
