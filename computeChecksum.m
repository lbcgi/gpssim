function D = computeChecksum(source, nib)  
    ...
    d = uint32(bitand(source , hex2dec('3FFFFFC0')));  
    D29 = uint32(bitand(bitshift(source , -31) , 1));  
    D30 = uint32(bitand(bitshift(source , -30) , 1));
  
    % Solve bits 23 and 24 to preserve parity check with zeros in bits 29 and 30  
    if nib  
        if mod(D30 + countBits(bitand(bmask(5) , d)) , 2) == 1  
            d = bitxor(d, 2^6);  
        end  
        if mod(D29 + countBits(bitand(bmask(6) , d)) , 2) == 1  
            d = bitxor(d, 2^7);  
        end  
    end  
    ...
    D = bitor(D, bitshift(mod((D30 + countBits(bitand(bmask(5), d))), 2), 1));
    D = bitor(D, mod((D29 + countBits(bitand(bmask(6), d))), 2));  
