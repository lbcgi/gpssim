...
% init: integer flag  
    ...
    % Define variables  
    g0.week = g.week;  
    g0.sec = g.sec + 0.5; % Align with the full frame length = 30 sec  
    ...
    if init == 1  
        ...
        for iwrd = 0:sysConfig.N_DWRD_SBF-1  
            ...
            chan.dwrd(iwrd + 1) = computeChecksum(sbfwrd + 1, nib);  
            prevwrd = chan.dwrd(iwrd + 1);  
        end  
    else
        for iwrd = 0:sysConfig.N_DWRD_SBF-1  
            chan.dwrd(iwrd + 1) = chan.dwrd(sysConfig.N_DWRD_SBF*sysConfig.N_SBF+iwrd + 1);  
            prevwrd = chan.dwrd(iwrd + 1);  
        end  
